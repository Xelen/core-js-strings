...
function formatTime(minutes, seconds) {
  const newMinutes = String(minutes);
  const newSeconds = String(seconds);
  return `${newMinutes.padStart(2, '0')}:${newSeconds.padStart(2, '0')}`;
}
...
function findLongestWord(sentence) {
  ...
  const maxQtyOfChars = Math.max(...newarr);
  const result = arr.find((item) => item.length === maxQtyOfChars);
  return result;
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const newarr = arr.map((item) => item.split('').reverse().join(''));
  return newarr.join(' ');
}
...
function invertCase(str) {
  return str.replace(/\w/g, function reducer(letter) {
